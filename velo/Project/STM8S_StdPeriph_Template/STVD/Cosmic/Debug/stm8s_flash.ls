...
1289                     ; 419   FlagStatus status = RESET;
1291                     ; 421   assert_param(IS_FLASH_FLAGS_OK(FLASH_FLAG));
1293                     ; 424   if((FLASH->IAPSR & (uint8_t)FLASH_FLAG) != (uint8_t)RESET)
1295  0001 c4505f        	and	a,20575
1296  0004 2702          	jreq	L325
1297                     ; 426     status = SET; /* FLASH_FLAG is set */
1299  0006 a601          	ld	a,#1
1302  0008               L325:
1303                     ; 430     status = RESET; /* FLASH_FLAG is reset*/
1306                     ; 434   return status;
1310  0008 5b01          	addw	sp,#1
1311  000a 81            	ret	
1396                     ; 549 IN_RAM(FLASH_Status_TypeDef FLASH_WaitForLastOperation(FLASH_MemType_TypeDef FLASH_MemType)) 
1396                     ; 550 {
1397                     .text:	section	.text,new
1398  0000               _FLASH_WaitForLastOperation:
1400  0000 5203          	subw	sp,#3
1401       00000003      OFST:	set	3
1404                     ; 551   uint8_t flagstatus = 0x00;
1406  0002 0f03          	clr	(OFST+0,sp)
1408                     ; 552   uint16_t timeout = OPERATION_TIMEOUT;
1410  0004 aeffff        	ldw	x,#65535
1412  0007 2008          	jra	L375
1413  0009               L765:
1414                     ; 578     flagstatus = (uint8_t)(FLASH->IAPSR & (FLASH_IAPSR_EOP | FLASH_IAPSR_WR_PG_DIS));
1416  0009 c6505f        	ld	a,20575
1417  000c a405          	and	a,#5
1418  000e 6b03          	ld	(OFST+0,sp),a
1420                     ; 579     timeout--;
1422  0010 5a            	decw	x
1423  0011               L375:
1424  0011 1f01          	ldw	(OFST-2,sp),x
1426                     ; 576   while((flagstatus == 0x00) && (timeout != 0x00))
1428  0013 7b03          	ld	a,(OFST+0,sp)
1429  0015 2604          	jrne	L775
1431  0017 1e01          	ldw	x,(OFST-2,sp)
1432  0019 26ee          	jrne	L765
1433  001b               L775:
1434                     ; 583   if(timeout == 0x00 )
1436  001b 1e01          	ldw	x,(OFST-2,sp)
1437  001d 2602          	jrne	L106
1438                     ; 585     flagstatus = FLASH_STATUS_TIMEOUT;
1440  001f a602          	ld	a,#2
1442  0021               L106:
1443                     ; 588   return((FLASH_Status_TypeDef)flagstatus);
1447  0021 5b03          	addw	sp,#3
1448  0023 81            	ret	
1511                     ; 598 IN_RAM(void FLASH_EraseBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType))
1511                     ; 599 {
1512                     .text:	section	.text,new
1513  0000               _FLASH_EraseBlock:
1515  0000 89            	pushw	x
1516  0001 5206          	subw	sp,#6
1517       00000006      OFST:	set	6
1520                     ; 600   uint32_t startaddress = 0;
1522                     ; 610   assert_param(IS_MEMORY_TYPE_OK(FLASH_MemType));
1524                     ; 611   if(FLASH_MemType == FLASH_MEMTYPE_PROG)
1526  0003 7b0b          	ld	a,(OFST+5,sp)
1527  0005 a1fd          	cp	a,#253
1528  0007 2605          	jrne	L536
1529                     ; 613     assert_param(IS_FLASH_PROG_BLOCK_NUMBER_OK(BlockNum));
1531                     ; 614     startaddress = FLASH_PROG_START_PHYSICAL_ADDRESS;
1533  0009 ae8000        	ldw	x,#32768
1535  000c 2003          	jra	L736
1536  000e               L536:
1537                     ; 618     assert_param(IS_FLASH_DATA_BLOCK_NUMBER_OK(BlockNum));
1539                     ; 619     startaddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
1541  000e ae4000        	ldw	x,#16384
1542  0011               L736:
1543  0011 1f05          	ldw	(OFST-1,sp),x
1544  0013 5f            	clrw	x
1545  0014 1f03          	ldw	(OFST-3,sp),x
1547                     ; 627     pwFlash = (PointerAttr uint32_t *)(MemoryAddressCast)(startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE));
1549  0016 a640          	ld	a,#64
1550  0018 1e07          	ldw	x,(OFST+1,sp)
1551  001a cd0000        	call	c_cmulx
1553  001d 96            	ldw	x,sp
1554  001e 1c0003        	addw	x,#OFST-3
1555  0021 cd0000        	call	c_ladd
1557  0024 be02          	ldw	x,c_lreg+2
1558  0026 1f01          	ldw	(OFST-5,sp),x
1560                     ; 631   FLASH->CR2 |= FLASH_CR2_ERASE;
1562  0028 721a505b      	bset	20571,#5
1563                     ; 632   FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NERASE);
1565  002c 721b505c      	bres	20572,#5
1566                     ; 636     *pwFlash = (uint32_t)0;
1568  0030 4f            	clr	a
...
1577  003a 81            	ret	
1681                     ; 655 IN_RAM(void FLASH_ProgramBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType, 
1681                     ; 656                         FLASH_ProgramMode_TypeDef FLASH_ProgMode, uint8_t *Buffer))
1681                     ; 657 {
...
1690                     ; 658   uint16_t Count = 0;
1692                     ; 659   uint32_t startaddress = 0;
1694                     ; 662   assert_param(IS_MEMORY_TYPE_OK(FLASH_MemType));
1696                     ; 663   assert_param(IS_FLASH_PROGRAM_MODE_OK(FLASH_ProgMode));
1698                     ; 664   if(FLASH_MemType == FLASH_MEMTYPE_PROG)
1700  0003 7b0b          	ld	a,(OFST+5,sp)
1701  0005 a1fd          	cp	a,#253
1702  0007 2605          	jrne	L317
1703                     ; 666     assert_param(IS_FLASH_PROG_BLOCK_NUMBER_OK(BlockNum));
1705                     ; 667     startaddress = FLASH_PROG_START_PHYSICAL_ADDRESS;
1707  0009 ae8000        	ldw	x,#32768
1709  000c 2003          	jra	L517
1710  000e               L317:
1711                     ; 671     assert_param(IS_FLASH_DATA_BLOCK_NUMBER_OK(BlockNum));
1713                     ; 672     startaddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
1715  000e ae4000        	ldw	x,#16384
1716  0011               L517:
1717  0011 1f03          	ldw	(OFST-3,sp),x
1718  0013 5f            	clrw	x
1719  0014 1f01          	ldw	(OFST-5,sp),x
1721                     ; 676   startaddress = startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE);
1723  0016 a640          	ld	a,#64
1724  0018 1e07          	ldw	x,(OFST+1,sp)
1725  001a cd0000        	call	c_cmulx
1727  001d 96            	ldw	x,sp
1728  001e 5c            	incw	x
1729  001f cd0000        	call	c_lgadd
1732                     ; 679   if(FLASH_ProgMode == FLASH_PROGRAMMODE_STANDARD)
1734  0022 7b0c          	ld	a,(OFST+6,sp)
1735  0024 260a          	jrne	L717
1736                     ; 682     FLASH->CR2 |= FLASH_CR2_PRG;
1738  0026 7210505b      	bset	20571,#0
1739                     ; 683     FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NPRG);
1741  002a 7211505c      	bres	20572,#0
1743  002e 2008          	jra	L127
1744  0030               L717:
1745                     ; 688     FLASH->CR2 |= FLASH_CR2_FPRG;
1747  0030 7218505b      	bset	20571,#4
1748                     ; 689     FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NFPRG);
1750  0034 7219505c      	bres	20572,#4
1751  0038               L127:
1752                     ; 693   for(Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
1754  0038 5f            	clrw	x
1755  0039 1f05          	ldw	(OFST-1,sp),x
1757  003b               L327:
1758                     ; 695     *((PointerAttr uint8_t*) (MemoryAddressCast)startaddress + Count) = ((uint8_t)(Buffer[Count]));
1760  003b 1e0d          	ldw	x,(OFST+7,sp)
1761  003d 72fb05        	addw	x,(OFST-1,sp)
1762  0040 f6            	ld	a,(x)
1763  0041 1e03          	ldw	x,(OFST-3,sp)
1764  0043 72fb05        	addw	x,(OFST-1,sp)
1765  0046 f7            	ld	(x),a
1766                     ; 693   for(Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
1768  0047 1e05          	ldw	x,(OFST-1,sp)
1769  0049 5c            	incw	x
1770  004a 1f05          	ldw	(OFST-1,sp),x
1774  004c a30040        	cpw	x,#64
1775  004f 25ea          	jrult	L327
1776                     ; 697 }
1779  0051 5b08          	addw	sp,#8
1780  0053 81            	ret	
1793                     	xdef	_FLASH_WaitForLastOperation
1794                     	xdef	_FLASH_ProgramBlock
1795                     	xdef	_FLASH_EraseBlock
1796                     	xdef	_FLASH_GetFlagStatus
1797                     	xdef	_FLASH_GetBootSize
1798                     	xdef	_FLASH_GetProgrammingTime
1799                     	xdef	_FLASH_GetLowPowerMode
1800                     	xdef	_FLASH_SetProgrammingTime
1801                     	xdef	_FLASH_SetLowPowerMode
1802                     	xdef	_FLASH_EraseOptionByte
1803                     	xdef	_FLASH_ProgramOptionByte
1804                     	xdef	_FLASH_ReadOptionByte
1805                     	xdef	_FLASH_ProgramWord
1806                     	xdef	_FLASH_ReadByte
1807                     	xdef	_FLASH_ProgramByte
1808                     	xdef	_FLASH_EraseByte
1809                     	xdef	_FLASH_ITConfig
1810                     	xdef	_FLASH_DeInit
1811                     	xdef	_FLASH_Lock
1812                     	xdef	_FLASH_Unlock
1813                     	xref.b	c_lreg
1814                     	xref.b	c_x
1815                     	xref.b	c_y
1834                     	xref	c_ladd
